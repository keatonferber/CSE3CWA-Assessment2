import { NextResponse } from 'next/server';
import { prisma } from '@/lib/prisma';
import { errorResponse, parseWordInput } from '@/lib/validation';
import { phonemeCreateData, wordInclude } from '@/lib/word-data';

export const dynamic = 'force-dynamic';

export async function GET(request) {
  const { searchParams } = new URL(request.url);
  const listId = Number(searchParams.get('listId'));
  const difficulty = searchParams.get('difficulty');
  const where = {};
  if (Number.isInteger(listId) && listId > 0) where.listId = listId;
  if (difficulty && ['easy', 'medium', 'hard'].includes(difficulty)) where.difficulty = difficulty;

  const words = await prisma.word.findMany({
    where,
    include: wordInclude,
    orderBy: [{ listId: 'asc' }, { english: 'asc' }]
  });
  return NextResponse.json(words);
}

export async function POST(request) {
  try {
    const input = parseWordInput(await request.json());
    const word = await prisma.word.create({
      data: {
        english: input.english,
        difficulty: input.difficulty,
        hint: input.hint,
        listId: input.listId,
        phonemes: { create: phonemeCreateData(input.phonemes) }
      },
      include: wordInclude
    });
    return NextResponse.json(word, { status: 201 });
  } catch (error) {
    const { status, message } = errorResponse(error);
    return NextResponse.json({ error: message }, { status });
  }
}
